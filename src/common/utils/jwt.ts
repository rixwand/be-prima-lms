...
export const signAccessToken = (sub: number, jti: string) => {
  return jwt.sign(
    { sub, jti, type: "access" } as JwtPayload,
    JWT_ACCESS_SECRET,
...
export const signRefreshToken = (sub: number, jti: string) => {
  return jwt.sign(
    { sub, jti, type: "refresh" } as JwtPayload,
    JWT_REFRESH_SECRET,
...
export const verifyAccessToken = (token: string): Result => {
  ...
  if (typeof res == "string") return [null, new Error(res)];
  else return [res as unknown as JwtPayload, null];
};
